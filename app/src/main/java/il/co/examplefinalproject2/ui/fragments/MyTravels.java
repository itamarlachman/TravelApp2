...
public class MyTravels extends Fragment {
    ...
    private MyTravelsViewModel myTravelsViewModel;

    public View onCreateView(@NonNull LayoutInflater inflater,
                             ViewGroup container, Bundle savedInstanceState) {
        myTravelsViewModel =
                ViewModelProviders.of(this).get(MyTravelsViewModel.class);
        View root = inflater.inflate(R.layout.my_travels, container, false);
        final TextView textView = root.findViewById(R.id.text_gallery);
        myTravelsViewModel.getText().observe(getViewLifecycleOwner(), new Observer<String>() {
            @Override
